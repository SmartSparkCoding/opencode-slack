...
- `!tempuser @user` grants that user temporary access to the current session (they can message the bot without being in `ALLOWED_USERS`). Works in active threads without `@bot`.
- `!sessions` lists all active opencode sessions with clickable links. Works in active threads without `@bot`.
- `!help` shows all available commands (ephemeral — only visible to you). The `!help` message itself is deleted. Works in active threads without `@bot`.
